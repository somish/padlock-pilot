...
function AdminInviteModal({ close, visible }) {
  ...
  let handleSubmit = async (e) => {
    ...
    try {
      await api.inviteAdmin(payload)
      setInviteSuccess(true)
      mixpanel.track('new_invite', { role: 'admin' })
    } catch (error) {
    ...
    <>
      <Modal show={visible} onHide={close}>
        <form onSubmit={handleSubmit}>
          <Modal.Header closeButton>Invite a new Administrator:</Modal.Header>
          <Modal.Body>
            <label>
              Email:
